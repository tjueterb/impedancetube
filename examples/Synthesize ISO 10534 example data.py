...
input = pf.signals.impulse(n_samples=4*fs, sampling_rate=fs, amplitude=0.7, delay=50)

# incident signal at position 1 (time delay can be omitted)
dirac = pf.signals.impulse(n_samples=1, sampling_rate=fs)
pos1_incident = pf.dsp.convolve(signal1=input, signal2=dirac)
